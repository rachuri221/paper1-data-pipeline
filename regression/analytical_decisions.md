...
## 14. Financial knowledge score vs. financial-education exposure — different predictor, deliberately not conflated

`analysis/11_knowledge_score_models.py` uses a completely different
predictor from every other model in this repo: a 0-5 "Big Five" financial
knowledge score (NFCS items `M6`-`M10`: compound interest, inflation, bond
prices, mortgage structure, diversification), not `M20` (financial-education
*exposure*). These are related but conceptually distinct — one measures
what someone *knows*, the other measures whether they *received a program*.
Knowledge is plausibly both a cause and a consequence of financial
behavior (and of financial education itself), so results from this model
should not be described as evidence about "financial education" in the
paper without that distinction being explicit.

Scoring convention: "Don't know" (NFCS code 98) is scored as an incorrect
answer (0 points), not treated as missing — this is the standard
convention in the financial-literacy literature (Lusardi/Mitchell and
FINRA's own NFCS reports both do this), since not knowing the answer is
substantively informative about financial knowledge, unlike declining to
answer. "Refused" (99) is treated as genuinely missing; a respondent
missing even one item due to refusal has their total score set to missing
rather than being scored on fewer than 5 items, so every non-missing score
in this analysis reflects exactly 5 answered items.

## 15. Sample definition: income-band cutoff instead of an approximate FPL proxy

The brief for this analysis called for restricting to "18-24 AND
underserved (≤200% FPL)." NFCS cannot support an exact FPL percentage: it
has no household-size variable anywhere in the instrument, and income is
collected in bands, not as a continuous dollar figure. Two paths were
considered:

1. **An approximate FPL proxy** — reconstruct household size from `A7`
   (living arrangement: alone / with spouse-partner / in parents' home /
   with other family-friends-roommates) plus `A11` (number of financially
   dependent children, top-coded at "4 or more"), then compare an income-
   band midpoint against the year-specific HHS poverty guideline for that
   reconstructed household size.
2. **A direct income-band cutoff** — household income under $35,000/year
   (NFCS bands 1-3: "<$15k," "$15-25k," "$25-35k" — identical cutoffs in
   both 2018 and 2021), with no household-size adjustment at all.

**Option 2 was chosen as more accurate for this specific population**, not
merely simpler. The FPL proxy's household-size reconstruction is weakest
exactly where this analysis needs it to be strongest: for an 18-24-year-old
living in their parents' home or with roommates (`A7` = 3 or 4), NFCS
provides no count of household members, and it's genuinely ambiguous
whether the income the respondent reports reflects their own finances or
the household they're living in. Building a poverty-percentage estimate on
top of that ambiguity would produce a number that looks more precise (a
specific % of FPL) while actually being less trustworthy than a transparent
income-band threshold — false precision is a worse failure mode than an
openly blunter measure. The income-band cutoff is exactly computable from
the variable NFCS actually collects, requires no imputation or proxying,
and is identically defined across both waves.

The tradeoff, stated plainly: this is a coarser definition of "underserved"
than a true FPL percentage would be. A large family at $34,000/year and a
single person at $34,000/year are both classified as "underserved" here,
even though the family is far more likely to be under 200% FPL in reality
and the single person's status is more borderline. If the team later
obtains a data source with actual household size (e.g., matching to Census
tract-level context, or a different survey instrument), the FPL-percentage
version should be revisited.
